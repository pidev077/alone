...
class Widget_FW_Login extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array( 'description' => esc_html__( 'A login form widget.', 'alone' ) );

		parent::__construct( false, esc_html__( 'Login - BT', 'alone' ), $widget_ops );
	}
	...
	function form( $instance ) {
		...
		<p>
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'show_remember' ) ); ?>"><?php esc_html_e( 'Show remember button', 'alone' ); ?></label><br/>
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'show_forgot' ) ); ?>"><?php esc_html_e( 'Show forgot password', 'alone' ); ?></label><br/>
		</p>
	<?php
	}
}