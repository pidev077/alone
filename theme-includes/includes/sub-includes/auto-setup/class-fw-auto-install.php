...
class FW_Simple_Auto_Install {
		...
		if ( is_array( $result ) ) {
			...
			if ( ! empty( $failed_extensions ) ) {
				...
				wp_send_json_error( array(
					'message' => sprintf( esc_html__( 'Failed to activate supported extensions. %s ', 'alone' ), implode( ', ', $failed_extensions ) ),
				) );
			}
		}
	...
	public function get_setup_messages() {
		return array(
			'plugins_only'      => sprintf( esc_html__( 'This option will activate Unyson dependencies. The %s demo content will not be installed %s.', 'alone' ), '<b>', '</b>' ),
			'plugins_and_demo'  => sprintf( esc_html__( 'This option will activate Unyson dependencies together %s with the demo content %s for the theme.', 'alone' ), '<b>', '</b>' ),
			'skip_auto_install' => esc_html__( 'Skip the auto setup all together and activate all the Unyson dependencies manually. Note that this page will not be  accessible until you install the theme again.', 'alone' )
		);
	}
	...
	private function admin_enqueue_scripts() {
		...
		$auto_setup_data = array(
			...
			'messages'                  => array(
				'on_leave_alert'    => esc_html__( 'Attention, the installation process is not yet finished, if leave this page, you lose the information stored on the site!', 'alone' ),
				'server_problems'   => esc_html__( 'Sorry, we\'ve encountered some errors, try to access this page later.', 'alone' ),
				'process_completed' => esc_html__( 'The installation process was completed successfully.', 'alone' )
			),
...
class FW_Admin_Auto_Install extends FW_Simple_Auto_Install {
		...
		if ( ! empty( $this->demos ) ) {
			...
			wp_localize_script(
				...
				array(
					...
					'messages'     => array(
						'installing'              => __( 'Installing', 'alone' ),
						'start_install_plugins'   => __( 'Installing required plugins ...', 'alone' ),
						'finish_install_plugins'  => __( 'Finished installing required plugins', 'alone' ),
						'start_activate_plugins'  => __( 'Activaiting required plugins ...', 'alone' ),
						'finish_activate_plugins' => __( 'Finished activaiting required plugins', 'alone' ),
					)
				)
			);
		}
	...
	public function get_demo_steps() {
		return array(
			'activate-demo-plugins' => array(
				...
				'message'     => esc_html__( 'Activate required demo plugins', 'alone' ),
			)
		);
	}
		...
		return array_merge(
			...
			array(
				'activate-unyson'           => array(
					...
					'message'     => esc_html__( 'Activating Unyson', 'alone' ),
				),
				'activate-required-plugins' => array(
					...
					'message'     => esc_html__( 'Activate required plugins', 'alone' ),
				)
			)
		);
	}
	...
	public function get_setup_messages() {
		return array(
			'plugins_only'      => sprintf( esc_html__( 'This option will activate %s already installed plugins and dependencies %s by Network Administrator. The %s demo content will not be installed %s.', 'alone' ), '<b>', '</b>', '<b>', '</b>' ),
			'plugins_and_demo'  => sprintf( esc_html__( 'This option will activate Unyson dependencies together %s with the demo content %s for the theme.', 'alone' ), '<b>', '</b>' ),
			'skip_auto_install' => esc_html__( 'Skip the auto setup all together and activate all the plugins manually. Note that this page will not be accessible until you install the theme again.', 'alone' )
		);
	}
	...
	public function activate_plugins( $plugins ) {
		if ( ! current_user_can( 'activate_plugins' ) ) {
			return new WP_Error( 0, __( 'Current user can\'t activate plugins', 'alone' ) );
		}
		...
		if ( ! empty( $failed_plugins ) ) {
			return new WP_Error( 0, sprintf(
					esc_html__( 'Failed to activate required plugins. %s', 'alone' ),
...
class FW_Super_Admin_Auto_Install extends FW_Admin_Auto_Install {
	...
	public function get_demo_steps() {
		return array(
			...
			'install-demo-plugins'  => array(
				...
				'message'     => esc_html__( 'Install required demo plugins', 'alone' ),
			),
		);
	}
	...
	public function install_demo_plugins() {
		...
		if ( ! $this->initialize_filesystem( $credentials, WP_PLUGIN_DIR ) ) {
			...
			wp_send_json_error( array(
				'message' => sprintf( esc_html__( 'Failed to install required plugins. Folder %s is not writable', 'alone' ), WP_PLUGIN_DIR ),
			) );
		}
		...
		if ( ! empty( $failed_plugins ) ) {
			...
			wp_send_json_error( array(
				'message' => sprintf( esc_html__( 'Failed to install required plugins. %s', 'alone' ), implode( ', ', $failed_plugins ) ),
			) );
		}
		ob_end_clean();
		wp_send_json_success();
		...
		return array_merge(
			...
			array(
				'install-unyson'               => array(
					...
					'message'     => esc_html__( 'Downloading and installing Unyson', 'alone' ),
				),
				'install-required-plugins'     => array(
					...
					'message'     => esc_html__( 'Installing required plugins', 'alone' ),
				),
				'install-supported-extensions' => array(
					...
					'message'     => esc_html__( 'Installing supported extensions', 'alone' ),
				),
				'install-child-theme'          => array(
					'ajax_action' => $this->_prefix . '_install_child_theme',
					'nonce'       => wp_create_nonce( 'install-child-theme' ),
					'message'     => esc_html__( 'Downloading and installing child theme', 'alone' ),
				),
			)
		);
	}
	...
	public function install_supported_extensions() {
		...
		if ( ! $this->initialize_filesystem( $credentials, WP_PLUGIN_DIR ) ) {
			wp_send_json_error( array(
				'message' => sprintf( esc_html__( 'Failed to install supported extensions. Folder %s is not writable', 'alone' ), WP_PLUGIN_DIR ),
			) );
		}
		...
		if ( is_array( $result ) ) {
			...
			if ( ! empty( $failed_extensions ) ) {
				...
				wp_send_json_error( array(
					'message' => sprintf( esc_html__( 'Failed to install supported extensions. %s ', 'alone' ), implode( ', ', $failed_extensions ) ),
				) );
			}
		}
	...
	public function install_unyson() {
		...
		if ( ! $this->initialize_filesystem( $credentials, WP_PLUGIN_DIR ) ) {
			wp_send_json_error( array(
				'message' => sprintf( esc_html__( 'Failed to install Unyson. Folder %s is not writable', 'alone' ), WP_PLUGIN_DIR ),
			) );
		}
	...
	public function install_child_theme() {
		...
		if ( ! $this->initialize_filesystem( $credentials, wp_normalize_path( get_theme_root() ) ) ) {
			wp_send_json_error( array(
				'message' => sprintf( esc_html__( 'Failed to install Theme. Folder %s is not writable', 'alone' ), wp_normalize_path( get_theme_root() ) ),
			) );
		}
	...
	public function get_setup_messages() {
		return array(
			'plugins_and_demo'  => sprintf( esc_html__( 'This option will install and activate all the plugins and dependencies together %s with the demo content %s for the theme.', 'alone' ), '<b>', '</b>' ),
			'plugins_only'      => sprintf( esc_html__( 'This option will install and activate all the plugins and dependencies. The %s demo content will not be installed %s.', 'alone' ), '<b>', '</b>' ),
			'skip_auto_install' => esc_html__( 'Skip the auto setup all together and install all the plugins manually. Note that this page will not be accessible until you install the theme again.', 'alone' )
		);
	}
}
